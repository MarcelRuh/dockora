...
> Status: **v2.0.17** – self-hosted Docker management with in-app updates and GHCR images.
...
**In the UI (recommended):** Sidebar → **Dockora** → *Jetzt aktualisieren* (admin).
...
```bash
wget -qO- https://raw.githubusercontent.com/MarcelRuh/dockora/main/scripts/install.sh | DOCKORA_USE_IMAGES=1 DOCKORA_IMAGE_TAG=2.0.17 bash
```
